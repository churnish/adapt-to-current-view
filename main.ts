...
export default class AccentColorPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.registerEvent(
      this.app.workspace.on('layout-change', () => this.updateAccentColor())
    );
    this.registerEvent(
      this.app.workspace.on('css-change', () => this.updateAccentColor())
    );
    this.updateAccentColor();
  }
  ...
  async loadSettings() {
    ...
    if (data && typeof data === 'object') {
      ...
      this.settings = {
        sourceColor: this.normalizeColor(
          str('sourceColor', DEFAULT_SETTINGS.sourceColor)
        ),
        livePreviewColor: this.normalizeColor(
          str('livePreviewColor', DEFAULT_SETTINGS.livePreviewColor)
        ),
        readingColor: this.normalizeColor(
          str('readingColor', DEFAULT_SETTINGS.readingColor)
        ),
        darkSourceColor: this.normalizeColor(
          str('darkSourceColor', DEFAULT_SETTINGS.darkSourceColor)
        ),
        darkLivePreviewColor: this.normalizeColor(
          str('darkLivePreviewColor', DEFAULT_SETTINGS.darkLivePreviewColor)
        ),
        darkReadingColor: this.normalizeColor(
          str('darkReadingColor', DEFAULT_SETTINGS.darkReadingColor)
        ),
      };
    } else {
  ...
  async saveSettings() {
    ...
    const cleanSettings: AccentColorSettings = {
      ...
      [SETTING_KEYS.darkLivePreviewColor]:
        this.settings.darkLivePreviewColor.replace('#', ''),
  ...
  updateAccentColor() {
    ...
    if (isDark) {
      if (mode === 'livePreview') {
        color =
          this.settings.darkLivePreviewColor ||
          this.settings.livePreviewColor ||
          '';
      } else if (mode === 'source') {
        color =
          this.settings.darkSourceColor || this.settings.sourceColor || '';
      } else {
        color =
          this.settings.darkReadingColor || this.settings.readingColor || '';
      }
    } else {
      if (mode === 'livePreview') {
  ...
  detectMode(view: MarkdownView): 'source' | 'livePreview' | 'reading' {
    ...
    if (
      readingView &&
      window.getComputedStyle(readingView).display !== 'none'
    ) {
      return 'reading';
    } else if (sourceView) {
  ...
  convertToHSL(color: string): [number, number, number] {
    ...
    if (
      ...
      !normalizedColor.match(/black|#000|rgb\(0,\s*0,\s*0\)/i)
    ) {
      return DEFAULT_HSL;
    }
...
class AccentColorSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Set accent color for each view mode when base color scheme is light.'
      )
      .setHeading();
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Set accent color for each view mode when base color scheme is dark. Leave blank to use light mode color.'
      )
      .setHeading();
    ...
    const buttonContainer = containerEl.createDiv({
      cls: 'setting-item adapt-restore-button-container',
    });
  ...
  private addColorSettings(container: HTMLElement, isDark: boolean): void {
    ...
    modes.forEach((mode) => {
      ...
      const setting = new Setting(container).setName(mode.name);

      let _textInput: TextComponent;
      ...
      setting
        .addColorPicker((color) => {
          ...
          colorPicker = color;
          color.setValue(currentValue || '#ffffff').onChange((value) => {
        ...
        .addText((text) => {
          ...
          _textInput = text;
          text
            .setPlaceholder(isDark ? '#FF0000' : '#FF0000')
            ...
            .onChange((value) => {
              ...
              if (colorPicker !== null && normalizedValue.length === 7) {
